...
def test(args, device_id, pt, step):
    ...
    # config = BertConfig.from_json_file(args.bert_config_path)
    model = Summarizer(args, device, load_pretrained_longformer=False, longformer_config = None)
    model.load_cp(checkpoint)
    model.eval()
...
def train(args, device_id):
    ...
    model = Summarizer(args, device, load_pretrained_longformer=True)
    if args.train_from != '':
...
if __name__ == '__main__':
    ...
    parser.add_argument("-result_path", default='../output/cnndm')
    ...
    parser.add_argument("-attention_window", default=[ 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256 ],)
    ...
    parser.add_argument("-top_n_sentence", default=3, type=int)
